		...
		if fromjid.getStripped() in COMMOFF and command in COMMOFF[fromjid.getStripped()]:
			...
			if fromjid.getStripped() in GROUPCHATS:
				if GCHCFGS[fromjid.getStripped()]['autoaway']==1:
					...
					LAST['gch'][fromjid.getStripped()]['thr']=threading.Timer(600,change_bot_status,(fromjid.getStripped(), u'я ничего здесь не делаю с '+time.strftime('%d.%m.%Y@%H:%M:%S GMT', time.gmtime()), 'away',1))
